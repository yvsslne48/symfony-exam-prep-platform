...
export default function ExamPage() {
  ...
  const accentColor = courseId === 'spring' ? 'var(--teal)' : 'var(--purple)'
  const accentLight = courseId === 'spring' ? '#2dd4a8' : '#a597ff'
      ...
      <div className="max-w-3xl mx-auto">
        <div className="text-center p-8 mb-6" style={{ ...ST }}>
          ...
          <p style={{ color:'var(--text2)', fontSize:14, marginBottom:20 }}>
            4 modes d'entraînement — EHEIO 2026
          </p>
          ...
          <div style={{ display:'flex', gap:24, justifyContent:'center', flexWrap:'wrap' }}>
            ...
            <Stat n={data.pdf.length} lbl="Questions PDF" color="var(--purple-light)" />
            {data.reel.map((r, i) => (
          ...
          {results.filter(r => (r.q.section||'Questions')===section).map(({q,ok,ud,cd},i) => (
            <div key={i} style={{ ...ST, padding:'1rem', marginBottom:8, borderLeft:`3px solid ${ok?'#2dd4a8':'#f06060'}` }}>
              <div style={{ display:'flex', alignItems:'flex-start', gap:8, marginBottom:10 }}>
                ...
                <div style={{ fontSize:13, fontWeight:500, lineHeight:1.5 }}>{q.q}</div>
              </div>
              {q.type==='open' ? (
                <>
