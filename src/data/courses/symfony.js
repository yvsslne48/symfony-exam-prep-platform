...
services:
    ...
    },
    {
      ...
      lessons: [
        {
          title: 'Créer une classe FormType',
          body: `<p>En Symfony, les formulaires sont des classes PHP qui héritent de <code>AbstractType</code>. On les génère avec la commande <code>make:form</code>.</p>`,
          code: `# Générer une classe formulaire :
...
class UserType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        $builder
            ...
            ->add('password', PasswordType::class, [
                'label'  => 'Mot de passe',
                'mapped' => false,  // Pas lié à une propriété de User
            ])
    ...
    public function configureOptions(OptionsResolver $resolver): void
    {
        $resolver->setDefaults([
            'data_class' => User::class,   // Lié à l'entité User
        ]);
    }
}`,
          language: 'php',
          tips: [
            'Hériter de AbstractType est OBLIGATOIRE',
            'buildForm() définit les champs du formulaire',
            'data_class → lie le formulaire à une entité',
            'mapped: false → champ non lié à une propriété de l\'entité',
          ]
        },
        {
          title: 'Traiter le formulaire dans le contrôleur',
          body: `<p>Dans le contrôleur, on crée le formulaire avec <code>createForm()</code>, on le traite avec <code>handleRequest()</code>, et on persiste les données si valide.</p>`,
          code: `<?php
use App\\Entity\\User;
...
class UserController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $em): Response
    {
        ...
        // 2. Lier la requête HTTP au formulaire
        $form->handleRequest($request);

        // 3. Vérifier soumis ET valide
        if ($form->isSubmitted() && $form->isValid()) {
            // $user est automatiquement rempli grâce à data_class
    ...
    // ÉDITION (même logique)
    #[Route('/users/{id}/edit', name: 'app_user_edit', methods: ['GET', 'POST'])]
    public function edit(User $user, Request $request, EntityManagerInterface $em): Response
    {
        $form = $this->createForm(UserType::class, $user); // pré-rempli
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $em->flush(); // pas besoin de persist() pour une MAJ !
            return $this->redirectToRoute('app_users_list');
        }
        ...
        return $this->render('user/edit.html.twig', ['form' => $form]);
          ...
          language: 'php',
          tips: [
            'createForm() → crée l\'instance du formulaire',
            'handleRequest() → lie les données POST au formulaire',
            'isSubmitted() → vérifie que le formulaire a été envoyé',
            'isValid() → vérifie les contraintes de validation',
            'Pour une MAJ : pas besoin de persist(), juste flush()',
          ]
        },
        {
          title: 'Affichage Twig et contraintes de validation',
          body: `<p>Dans Twig, on affiche le formulaire avec des fonctions dédiées. Les contraintes de validation se placent directement sur l'entité.</p>`,
          code: `{# templates/user/new.html.twig #}
...
{{ form_start(form) }}
    ...
    {# OU séparément pour plus de contrôle : #}
...
<?php
// src/Entity/User.php — CONTRAINTES DE VALIDATION
...
class User
{
    ...
    private string $password;
          ...
          language: 'twig',
          note: '✅ form_row() = form_label() + form_widget() + form_errors()\n\nLes fonctions à connaître :\n• form_row() → tout-en-un (recommandé)\n• form_widget() → seulement l\'input\n• form_label() → seulement le label\n• form_errors() → seulement les erreurs\n• form_start() / form_end() → balise <form> (form_end ajoute le token CSRF)',
          tips: [
            'form_row() est la méthode recommandée (tout-en-un)',
            'Les contraintes se mettent sur l\'ENTITÉ (pas le formulaire)',
            '#[Assert\\NotBlank] → champ obligatoire',
            '#[Assert\\Email] → format email valide',
            'form_end() génère automatiquement le token CSRF',
          ]
        }
      ],
      quiz: [
        { q: 'De quelle classe PHP héritent tous les formulaires Symfony ?', opts: ['AbstractForm', 'BaseType', 'AbstractType', 'FormBase'], ans: 2 },
        { q: 'Quelle méthode du contrôleur crée une instance de formulaire ?', opts: ['makeForm()', 'buildForm()', 'createForm()', 'newForm()'], ans: 2 },
        { q: 'Comment vérifier qu\'un formulaire est soumis ET valide ?', opts: ['$form->check()', '$form->isSubmitted() && $form->isValid()', '$form->validate()', '$form->isOk()'], ans: 1 },
        { q: 'Laquelle de ces fonctions Twig affiche label + input + erreurs ?', opts: ['form_widget()', 'form_label()', 'form_all()', 'form_row()'], ans: 3 },
        { q: 'Quelle contrainte Symfony vérifie qu\'un champ n\'est pas vide ?', opts: ['#[Assert\\Required]', '#[Assert\\NotBlank]', '#[Assert\\NotNull]', '#[Assert\\Filled]'], ans: 1 },
        { q: 'Où se placent les contraintes de validation en Symfony ?', opts: ['Dans le FormType', 'Dans le contrôleur', 'Sur l\'entité', 'Dans services.yaml'], ans: 2 },
        { q: 'Pour mettre à jour un enregistrement, faut-il appeler persist() ?', opts: ['Oui, toujours', 'Non, seulement flush() suffit si l\'entité est déjà gérée', 'Oui, puis flush()', 'Non, la mise à jour est automatique'], ans: 1 },
        { q: 'À quoi sert "mapped: false" dans un champ de formulaire ?', opts: ['Désactive la validation', 'Le champ n\'est pas lié à une propriété de l\'entité', 'Cache le champ', 'Rend le champ optionnel'], ans: 1 },
      ],
      exam: [
        { type: 'dropdown', q: 'Depuis quelle classe devons-nous étendre nos formulaires avant de les utiliser ?', opts: ['AbstractForm', 'BaseFormType', 'AbstractType', 'FormBuilder'], ans: 2, hint: 'AbstractType — dans Symfony\\Component\\Form\\AbstractType' },
        { type: 'open', q: 'Écrire le nom de la méthode qu\'on utilise pour créer un formulaire dans le contrôleur', ans: 'createForm(NomFormType::class, $entity) — Méthode de AbstractController qui instancie le formulaire et le lie à l\'entité.', hint: 'createForm(UserType::class, $user)' },
        { type: 'checkbox', q: 'Parmi les fonctions Twig suivantes, sélectionner la ou les fonctions qui servent à afficher le formulaire', opts: ['form_row', 'form_end', 'form_widget', 'form_field_label', 'form_label', 'form_attributes'], ans: [0, 1, 2, 4], hint: 'form_row (label+input+erreurs), form_end (</form>+CSRF), form_widget (input seul), form_label (label seul). form_field_label et form_attributes n\'existent pas.' },
        { type: 'open', q: 'Donner 2 façons pour valider un formulaire en Symfony', ans: '1. $form->isSubmitted() && $form->isValid() — vérification dans le contrôleur\n2. Les contraintes de validation (#[Assert\\NotBlank], #[Assert\\Email]...) sur les propriétés de l\'entité', hint: 'isSubmitted() && isValid() + contraintes #[Assert\\...] sur l\'entité.' },
        { type: 'open', q: 'Pour enregistrer les données en base de données, nous devons s\'assurer que le formulaire est soumis. Donner le nom de la méthode à appeler', ans: 'isSubmitted() — $form->isSubmitted() vérifie si le formulaire a bien été envoyé (méthode POST). À combiner avec isValid().', hint: '$form->isSubmitted()' },
        { type: 'open', q: 'Écrire 2 contraintes de validation vues dans le cours et permettant de valider un champ', ans: '#[Assert\\NotBlank] — vérifie que le champ n\'est pas vide\n#[Assert\\Email] — vérifie que le format email est valide\n(autres valides : #[Assert\\Length], #[Assert\\Regex], #[Assert\\PositiveOrZero], #[Assert\\Url]...)', hint: '#[Assert\\NotBlank], #[Assert\\Email], #[Assert\\Length], #[Assert\\Regex]...' },
      ]
    },
    {
      id: 'doctrine',
